...
1. Monetization. Monetizable contests are only possible by building onchain. There is no way to programmatically offer rewards to winners offchain. And just as importantly, there is no way to monetize contests directly offchain either—whereas onchain, we can split all commissions with you 50/50 so that everyone can earn. And that’s with 0 credit card or appstore fees that you’d incur offchain. All of this means we are the only way for projects to build a viable business through contests.
...
### How can I monetize by creating contests?
...
JokeRace currently supports frames on these chains:
- Ethereum mainnet
- Base
- Optimism
- Zora
